...
class ApiV1::ScrobblesTest < ActionDispatch::IntegrationTest
  ...
  test 'should send now_playing info to last.fm at /songs/now_playing' do
    ...
    put '/api/v1/songs/now_playing?artist=Silence&title=30%20second%20silence', { :format => 'json' }, 'rack.session' => session, 'rack.session.options' => {}
  ...
  test 'should send scrobble to last.fm at /songs/scrobble' do
    ...
    post '/api/v1/songs/scrobble?artist=Silence&title=30%20second%20silence', { :format => 'json' }, 'rack.session' => session, 'rack.session.options' => {}
  ...
  test 'should return error if wrong HTTP method' do
    assert_raises ActionController::RoutingError do
      get '/api/v1/songs/scrobble?title=30%20second%20silence', { :format => 'json' }, 'rack.session' => session, 'rack.session.options' => {}
    end
  end
  ...
  test 'should return error if artist is missing' do
    post '/api/v1/songs/scrobble?title=30%20second%20silence', { :format => 'json' }, 'rack.session' => session, 'rack.session.options' => {}
    assert_response 422
  end

  test 'should return error if title is missing' do
    post '/api/v1/songs/scrobble?artist=Silence', { :format => 'json' }, 'rack.session' => session, 'rack.session.options' => {}
    assert_response 422
  end
end
